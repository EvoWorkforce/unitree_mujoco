...
// External goal pose from main.cc (x, y, yaw)
extern std::vector<double> g_goal_pose;
extern bool g_goal_pose_updated;

// External key press from main.cc
extern std::string g_key_press;
extern bool g_key_press_updated;
...
public:
    ...
    void printSceneInformation()
    {
        ...
        printObjects("Link", mj_model_->nbody, mjOBJ_BODY, [](int i)
                     { return i; });
        printObjects("Joint", mj_model_->njnt, mjOBJ_JOINT, [](int i)
                     { return i; });
        printObjects("Actuator", mj_model_->nu, mjOBJ_ACTUATOR, [](int i)
                     { return i; });
...
class RobotBridge : public UnitreeSDK2BridgeBase
{
    ...
    using GoalPose_t = unitree::robot::RealTimePublisher<geometry_msgs::msg::dds_::Pose2D_>;
    using KeyPress_t = unitree::robot::RealTimePublisher<std_msgs::msg::dds_::String_>;
...
public:
    RobotBridge(mjModel *model, mjData *data) : UnitreeSDK2BridgeBase(model, data)
    {
        ...
        goal_pose_pub = std::make_unique<GoalPose_t>("rt/goal_pose");
        key_press_pub = std::make_unique<KeyPress_t>("rt/key_press");
    }

    void start()
    {
        thread_ = std::make_shared<unitree::common::RecurrentThread>(
            "unitree_bridge", UT_CPU_ID_NONE, 1000, [this]()
            { this->run(); });
    }
    ...
    virtual void run()
    {
        ...
        // goal_pose (x, y, yaw) - publish when updated
        if (goal_pose_pub->trylock())
        {
            goal_pose_pub->msg_.x(g_goal_pose[0]);
            goal_pose_pub->msg_.y(g_goal_pose[1]);
            goal_pose_pub->msg_.theta(g_goal_pose[2]);
            goal_pose_pub->unlockAndPublish();
        }
        // key_press - publish when updated
        if (g_key_press_updated && key_press_pub->trylock())
        {
            key_press_pub->msg_.data(g_key_press);
            key_press_pub->unlockAndPublish();
            g_key_press_updated = false;
        }
    }
    ...
    std::unique_ptr<GoalPose_t> goal_pose_pub;
    std::unique_ptr<KeyPress_t> key_press_pub;
